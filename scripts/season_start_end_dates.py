...
#     # Add Years and dates into lists
...
#             # fourth td → grab divs, pick last one
#             divs = tds[3].find_all("div")
#             if divs:  
#                 end_date = divs[-1].get_text(strip=True)
#             else:
#                 end_date = tds[3].get_text(strip=True)  # fallback
...
#     # Compute Start date
#     for year, end_str in zip(years, end_dates):
#         # Ensure the string includes the year (assumes format like "June 24, 2026")
#         clean_str = end_str if year in end_str else f"{end_str}, {year}"
#         # Parse to date; assumes no slash in the day
#         end_date = datetime.strptime(clean_str, "%A, %B %d, %Y").date()
...
# scripts/season_start_end_dates.py
import sys
import re
